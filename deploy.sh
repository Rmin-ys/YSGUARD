...
if [ "$(date +%H:%M)" == "00:00" ]; then
    ...
    TD=$(( $(cat /etc/total_down 2>/dev/null || echo 0) + ${CD:-0} )); TU=$(( $(cat /etc/total_up 2>/dev/null || echo 0) + ${CU:-0} ))
    [ -n "$TOKEN" ] && curl -sk --connect-timeout 5 -X POST "$TG_URL/bot$TOKEN/sendMessage" -d "chat_id=$CHATID" -d "text=text=📊 Manual Daily Report%0A🖥 Host: $(hostname)%0A🔄 Resets: $RC%0A📥 Total Down: $((TD/1048576)) MB%0A📤 Total Up: $((TU/1048576)) MB" -d "parse_mode=Markdown" >/dev/null 2>&1 &
